...
class Persistence:
    ...
    async def save_order(self, order: Order) -> None:
        await asyncio.to_thread(self._save_order_sync, order)
    ...
    async def save_fill(self, fill: Fill) -> None:
        await asyncio.to_thread(self._save_fill_sync, fill)
    ...
    async def get_recent_fills(self, limit: int = 100) -> list[dict[str, Any]]:
        return await asyncio.to_thread(self._get_recent_fills_sync, limit)
    ...
    async def save_pnl_snapshot(
        ...
        await asyncio.to_thread(self._save_pnl_sync, realized, unrealized, fees)
    ...
    async def save_inventory_snapshot(self, position: Position) -> None:
        await asyncio.to_thread(self._save_inventory_sync, position)
    ...
    async def get_recent_events(self, limit: int = 50) -> list[dict[str, Any]]:
        return await asyncio.to_thread(self._get_recent_events_sync, limit)
